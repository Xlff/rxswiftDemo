...
final class SearchViewModel: ViewModelType {
    ...
    struct Dependencies {
        let api: ApiProvider
        let navigator: SearchMovieNavigator
    }
    ...
    func transform(input: SearchViewModel.Input) -> SearchViewModel.Output {
        ...
        let results = Observable.combineLatest(query, selectedCategoryIndex)
            .flatMapLatest { pair -> Observable<[SearchResultItem]> in
                ...
                switch type {
                case .movies:
                    return self.dependencies.api.searchMovies(forQuery: query)
        ...
        let emptyResult = input.searchText
            .asObservable()
            .filter { $0.count < 3 }
            .map { _ in return [SearchResultItem]() }
        let mappedResults = Observable.merge(results, emptyResult)
        ...
        let selectedDone = input.selected
            .asObservable()
            .withLatestFrom(Observable.merge(results, emptyResult)) {
                indexPath, results in
            ...
            .do(onNext: { [weak self] result in
                ...
                switch result.type {
                case .movies:
                    strongSelf.dependencies.navigator.navigateToMovieDetailScreen(withId: result.id, api: strongSelf.dependencies.api)
                case .people:
